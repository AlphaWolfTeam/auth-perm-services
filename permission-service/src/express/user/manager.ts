...
export class AppManager {
    ...
    static async addNewUser(user: Omit<IUser, "name">) {
        ...
        console.log('Mongo: New user saved to collection');
        return true;
    }
    ...
    static async updateUser(user: Omit<IUser, "name">) {
        if (permissionNumRangeCheck(user.permission)) {
            ...
            if (res.lastErrorObject.updatedExisting) {
                ...
                console.log('Mongo: User data updated');
            ...
            console.error("TypeError: Permission is not in range");
            return false;
        }
    }
    ...
    static async deleteUser(adfsId: String) {
        ...
        if(result.deletedCount) {
            console.log('Mongo: User deleted successfully');
                return { status: 'success', adfsId: adfsId };
        } else {
            console.error("MongoError: User not found");
            return false;
        }
    }
